...
package net.mcreator.gra.world.dimension;
...
import net.mcreator.gra.particle.EnderForestParticleParticle;
import net.mcreator.gra.item.EndNewItem;
...
public class EndNewDimension extends GraModElements.ModElement {
	@ObjectHolder("gra:end_new_portal")
	public static final CustomPortalBlock portal = null;
	public EndNewDimension(GraModElements instance) {
		super(instance, 105);
	...
	public void clientLoad(FMLClientSetupEvent event) {
		DimensionRenderInfo customEffect = new DimensionRenderInfo(Float.NaN, true, DimensionRenderInfo.FogType.NONE, false, false) {
			...
			public Vector3d func_230494_a_(Vector3d color, float sunHeight) {
				return new Vector3d(0.070588235294, 0, 0.070588235294);
			}
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("gra:end_new"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("end_new_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("end_new_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("end_new_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new EndNewItem().setRegistryName("end_new"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			for (int i = 0; i < 4; i++) {
				...
				world.addParticle(EnderForestParticleParticle.particle, px, py, pz, vx, vy, vz);
			}
		...
		public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				...
				} else if (entity.world.getDimensionKey() != RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("gra:end_new"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("gra:end_new")));
				} else {
					entity.func_242279_ag();
